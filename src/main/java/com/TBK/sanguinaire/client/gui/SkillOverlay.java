...
public class SkillOverlay implements IGuiOverlay {
    protected static final ResourceLocation WIDGETS_LOCATION = new ResourceLocation(Sanguinaire.MODID,"textures/gui/skill_hotbar.png");
        ...
        if (this.mc.player != null) {
            ...
            if(cap!=null && cap.getPlayerVampire().isVampire()){
                ...
                if(cap.hotbarActive){
                    ...
                    graphics.blit(WIDGETS_LOCATION, i + 96 + cap.getPosSelectSkillAbstract() *20, height - 60 , 20, 32, 24, 21
                            ,256,256);
                }else {
                    graphics.blit(WIDGETS_LOCATION, i + (int)Sanguinaire.x + 132, height - 74 + (int)Sanguinaire.y , 0, 34, 18, 32
                            ,256,256);
                }
                graphics.pose().popPose();
